...
const page = ({params}) => {
  ...
  const [url, seturl] = useState(	urll)
    ...
    const getSingle = async ()=>{
      const {data} = await axios.get(`/movie/${params.id}?api_key=b66939d4f30a0c82419ffe7f977d0768&language=en-US&append_to_response=credits`)
      ...
      crew.map((index)=>{
         ...
         index.job === "Story" ?
         setcreww(creww => ([...creww ,index])) :

         index.job === "Characters" ?
         setcreww(creww => ([...creww ,index])) :

         index.job ==="Director" ?
         setcreww(creww => ([...creww ,index])) :

         index.job === "Screenplay" ? 
         setcreww(creww => ([...creww ,index])) :
            ...
            <div className='right-title'>
              <div className='right-title-top'>
                ...
                {genres.map((index)=>(
                 <span key={index.id}> {index.name}, </span> 
                  ...
                  • {timeConvert(movie.runtime)}</span>
              </div>
              <div className='icons'>
              ...
              <div className='cast'>
                {creww.slice(0,6).map((index)=>(
                  <div className='artist' key={index.id + index.job}>
